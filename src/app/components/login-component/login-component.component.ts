  ...
  tryLogin(uname: string, pass: string, ): void {
    ...
    this.postloginService.verifyUser(user2).subscribe((response) => {
      ...
      if(response[0] != null){
        ...
        window.localStorage.setItem("username", JSON.stringify(response[0].username));
        window.localStorage.setItem("email", JSON.stringify(response[0].email));
        window.localStorage.setItem("birthday", JSON.stringify(response[0].birthday));
        window.localStorage.setItem("favMovies", JSON.stringify(response[0].favoriteMovies[0].id ));
        console.log(window.localStorage.getItem("favMovies"));  
        window.localStorage.setItem("wishlist", JSON.stringify(response[0].wishList[0]));
